...
public class MainMenuScreen implements Screen {
    ...
    public MainMenuScreen(boolean isQuick) {
    ...
    public void show() {
        ...
        playButton.addListener(new ClickListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                highScoresButton.removeListener(highScoresButton.getListeners().first());
                settingsButton.removeListener(settingsButton.getListeners().first());
        ...
        playButtonStage.addAction(sequence(fadeOut(0f),
                moveTo(Constants.WINDOW_WIDTH - playButton.getWidth() * playButton.getScaleX() - 10, 90),delay(0.5f), fadeIn(1.5f),
                forever(sequence(fadeOut(0.75f, Interpolation.exp5), fadeIn(0.75f, Interpolation.exp5), delay(1.0f)))));

//                forever(parallel(sequence(scaleTo(1.05f, 1.05f, 0.25f), scaleTo(1.00f, 1.00f, 0.25f)),
//                        sequence(moveBy(-1.9f,-1, 0.25f),moveBy(1.9f,1, 0.25f))))));
        ...
        demoButtonStage.addAction(sequence(fadeOut(0f),
                moveTo(Constants.WINDOW_WIDTH - playButton.getWidth() * playButton.getScaleX() - 10, 55),
                delay(0.5f),fadeIn(1.5f)));
        ...
        // High Scores Button
        highScoresButton = new Image(Constants.scoresButton);
        highScoresButton.setScale(0.15f);
        highScoresButton.setBounds(highScoresButton.getX(), highScoresButton.getY(), highScoresButton.getWidth(), highScoresButton.getHeight());
        ...
        highScoresStage = new Stage(viewport);
        highScoresStage.addAction(sequence(fadeOut(0f),moveTo(10, 90),
                delay(0.5f),fadeIn(1.5f)));
        highScoresStage.addActor(highScoresButton);
        highScoresButton.setTouchable(Touchable.enabled);
        inputMultiplexer.addProcessor(highScoresStage);

        // Settings Button
        settingsButton = new Image(Constants.settingsButton);
        settingsButton.setScale(0.15f);
        settingsButton.setBounds(settingsButton.getX(), settingsButton.getY(), settingsButton.getWidth(), settingsButton.getHeight());
        settingsButton.addListener(new ClickListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                highScoresButton.removeListener(highScoresButton.getListeners().first());
                settingsButton.removeListener(settingsButton.getListeners().first());
                Constants.game.setScreen(new SettingsScreen());
                return true;
            }
        });
        settingsStage = new Stage(viewport);
        settingsStage.addAction(sequence(fadeOut(0f),moveTo(10, 55),
                delay(0.5f),fadeIn(1.5f)));
    ...
    public void render(float delta) {
        ...
        if (group3.getActions().size == 0 && introStep == 4){
            ...
            if (z > 2f){
                ...
                stage.addAction(sequence(moveTo(1,Constants.WINDOW_HEIGHT + 5),delay(0f), moveTo(1, 5, 1f)));

                stage2.addActor(name);
                stage2.addAction(sequence(moveTo(10,Constants.WINDOW_HEIGHT + 15), delay(0.5f),
                        moveTo(10,Constants.WINDOW_HEIGHT - name.getHeight()*name.getScaleY() - 5, 0.5f)));
            }
        }
         if (introStep == 5) {
            stage3.getViewport().apply();
            ...
            highScoresStage.getViewport().apply();
            highScoresStage.act();
